...
# 이 사람들은 혈압이 population 기준(수축기 <=100/>=140, 이완기 >=90)에
# 거의 매번 걸리지만, 그게 그 사람의 평소 상태라 "특이사항"이 아니다
# (docs/daily-report-workflow.md 7번, scripts/baseline_check.py로 산출).
# 혈압 수치만으로는 태그를 붙이지 않고, 실제 증상이 함께 있을 때만 주의로 잡는다.
CHRONIC_LOW_SYS = {"조성예", "남을희", "정금자", "박분선"}
CHRONIC_HIGH_SYS = {"홍진표", "이희자"}
CHRONIC_HIGH_DIA = {"이희자"}
...
def judge(vital, note, name=None):
